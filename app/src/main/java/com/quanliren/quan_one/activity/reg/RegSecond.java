...
public class RegSecond extends BaseActivity {
    ...
    public void init() {
        ...
        setListener();
        invite_code.setTransformationMethod(HideReturnsTransformationMethod.getInstance());
        if (deviceType == 0) {
            invite_ll.setVisibility(View.VISIBLE);
        } else if (deviceType == 1) {
            invite_ll.setVisibility(View.GONE);
        }
    }
    ...
    public void rightClick() {
        if (canPost.get()) {
            ...
            ap.put("dUnique", ac.cs.getDeviceId());
            ap.put("dtype", 0);
            if (deviceType == 0) {
                if (!TextUtils.isEmpty(invite_code.getText().toString())) {
                    ap.put("inviteCode", invite_code.getText().toString().trim());
                }
                ap.put("dUnique", ac.cs.getDeviceId());
            }
            User lou = new User();
