...
//        int widthSpecMode = MeasureSpec.getMode(widthMeasureSpec);
    ...
    private int[] onUpdateColorFactory(int position) {
        int[] colors;
        ...
        } else if (mTheme == ColorFactory.APPOINT) {
            colors = new int[]{Color.parseColor(mTags.get(position).getColor()), Color.parseColor(mTags.get(position).getColor()), Color.parseColor(mTags.get(position).getTextColor())};
        } else {
    ...
    private void onSetTag() {
        ...
        for (int i = 0; i < mTags.size(); i++) {
            onAddTag(mTags.get(i).text, mChildViews.size());
        }
        postInvalidate();
    }
    ...
    private void initTagView(TagView tagView, int position) {
        int[] colors = onUpdateColorFactory(position);
    ...
    public void addTag(TagBean tag) {
        mTags.add(tag);
        addTag(tag.text, mChildViews.size());
    }
